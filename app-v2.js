...
const strings = japanese
  ? { slow: '遅い', fast: '速い', play: '再生', stop: '停止', game: 'ハエ叩き', hit: '叩く' }
  : { slow: 'SLOW', fast: 'FAST', play: 'PLAY', stop: 'STOP', game: 'Fly swatter', hit: 'HIT' };
...
hitButton.textContent = strings.hit;
...
document.addEventListener('touchmove', (event) => {
  if (gameEnabled || event.touches.length > 1) event.preventDefault();
}, { passive: false });
game.addEventListener('touchstart', blockGesture, { passive: false });
game.addEventListener('touchmove', blockGesture, { passive: false });
...
function renderBrain(now) {
  ...
  for (let node = 0; node < flyData.nodeCount; node += 1) {
    ...
    brainCtx.fillRect(Math.round(p[0] - 2), Math.round(p[1] - 2), 4, 4);
  }
}
...
function animateGame(now) {
  ...
  flyX += (flyTargetX - flyX) * follow;
  flyY += (flyTargetY - flyY) * follow;
...
async function initializeAudio() {
  ...
  try {
    ...
    await Promise.all([resumePromise, audioContext.audioWorklet.addModule(WORKLET_URL)]);
    ...
    audioNode.port.onmessage = (event) => {
      if (event.data?.type === 'events') handleEvents(event.data.events);
    ...
    audioNode.port.postMessage({
      type: 'init', wasm: wasmBuffer, piano: pianoBuffer, voice: voiceBuffer, graph: graphBuffer,
      seed: randomSeed(), bpm: tempoBpm,
